(function () {
  ...
  document.querySelectorAll(".sec3__video").forEach(autoplay);

  /* ---- business cards: video plays on hover, frozen frame otherwise ------- */
  document.querySelectorAll(".bizcard").forEach((card) => {
    const v = card.querySelector(".bizcard__video");
    if (!v) return;
    v.muted = true;
    v.playsInline = true;
    // render a still frame so the resting card isn't blank
    const prime = () => { try { if (v.paused) v.currentTime = 0.1; } catch (e) {} };
  ...
  (function scrollReveal() {
    ...
    sections.forEach((sec) => {
      ...
      // inside a capitem the lines (word -> label -> sub) get a per-line stagger
      // so the two right lines clearly rise one at a time
      sec.querySelectorAll(".hero-line-inner, .stat__num, .stat__clip > .stat__label, .stat__clip > .stat__desc")
        .forEach((el) => {
          const cap = el.closest ? el.closest(".capitem") : null;
          let stagger = 0;
          if (cap) {
            const lines = Array.prototype.slice.call(cap.querySelectorAll(".hero-line-inner"));
            stagger = Math.max(0, lines.indexOf(el)) * 0.2;
          }
          targets.push({ el, anchor: el.parentElement, kind: "line", stagger: stagger, p: 0 });
        });
      // tags rise + fade in
      ...
      // business cards rise from below one at a time, then enable hover accordion
    ...
    if (reduced) {
      targets.forEach((t) => {
        ...
        if (t.kind === "card") t.el.classList.add("is-visible");
      });
      return;
    }
    ...
    function frame() {
      ...
      for (let i = 0; i < targets.length; i++) {
        ...
        if (t.stagger) {
          // delay this line's start by its stagger, then rescale to still finish at p=1
          p = (p - t.stagger) / (1 - t.stagger);
          if (p < 0) p = 0; else if (p > 1) p = 1;
        }
        if (t.kind === "card") {
        ...
        if (t.kind === "line") {
          ...
          if (p >= 0.999) {
            ...
            t.el.style.opacity = "";
            t.el.style.transition = "";          // restore CSS (hover accordion)
